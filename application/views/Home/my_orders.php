...
<!---Php Meassge Show --->
	...
	<div class="card">
		<div class="card-content" style="border-bottom: 1px solid silver;">
			...
			<?php elseif($order->order_status == "Cancel"): ?>	
				...
				<?php else: ?>
					...
					<a href="<?= base_url('Home/reason_cancel_orders/' .$order->order_id); ?>"  class="btn waves-effect waves-light right" style="background: no-repeat;color: red;border: 1px solid silver;box-shadow: none;"> Cancel  
		...
		<div class="card-content" style="border-bottom: 1px solid silver;">
			...
			<div class="row">
				...
				<div class="col l5 m5 s12">
					<h6 style="color: grey;font-weight: 500;font-size: 14px;"><?= $ord_item->product_name; ?></h6>
					<h6 style="color: orange;font-weight: 500;font-size: 14px;">Quantity : <?= $ord_item->quantity; ?></h6>
				...
				<div class="col l5 m4 s12">
					...
					<?php if ($order->order_status == "Delivered"):
						...
						$status .= '<a href="'.base_url("Home/manage_rating/").$order->id.'"> &nbsp;
						...
						<?php elseif ($order->order_status == "Packed"):
							# code...
							$status = '<h6 style="color:rgba(212, 245, 66);font-size: 15px;font-weight: 500">Your Order is : Packed</h6>';
						?>
						<?php elseif ($order->order_status == "Dispatch"):
							# code...
							$status = '<h6 style="color:rgba(245, 233, 66);font-size: 15px;font-weight: 500">Your Order is : Dispatch</h6>';
						?>	
						<?php elseif ($order->order_status == "Cancel"):
							# code...
						...
						?> 
						<?php else: 
							$status = '<h6 style="color:rgba(245, 93, 66);font-size: 15px;font-weight: 500">Your Order is : Pending</h6>';
							?>
						<?php endif; ?>
		...
		<div class="card-content" style="padding: 10px;">
			...
			<h6 style="color: grey;margin-top: 5px;">Order On : <b><?= date('D, M. d Y',strtotime($order->order_date)); ?></b> <span class="right">Order Total : <b><span class="fa fa-rupee-sign">&nbsp;
				<?= number_format($order->total_amount); ?></span></b></span>
				<h6 style="color: green;text-align: center;font-weight: 500;font-size: 15px;"  class="right">
					<span style="color: red">&nbsp;Discount Price :<span class="fa fa-rupee-sign"></span>&nbsp;<?= number_format($order->coupan_value); ?></span><br>
				Discount Final Price : <span class="fa fa-rupee-sign">&nbsp;<?= number_format($order->final_price); ?></span>
			</h6>
			</h6><br><br>
		</div>
	</div>
